...
class Solver:
    ...
    def get_maxh_table(self):
        ...
        self.table_comparisons = temp_df.merge(temp_df, on='key', suffixes=('_guess', '_answer')).drop('key', axis=1)
        self.table_comparisons.rename(columns={'rivers_guess': 'guess', 'rivers_answer': 'answer'}, inplace=True)
        self.table_comparisons['table_comparison'] = self.table_comparisons.apply(
            lambda row: self.compare_tables(row['guess'], row['answer']),
            axis=1
        )
        self.table_comparisons[['guess_str', 'answer_str']] = self.table_comparisons[['guess', 'answer']].astype(str)
        self.comparisons_matrix = self.table_comparisons.pivot(index='answer_str', columns='guess_str', values='table_comparison')

        self.river_entropies = self.comparisons_matrix.apply(self.entropy_from_series, axis=0).sort_values(ascending=False)

        maxh_table_str = self.river_entropies.index[0]
        self.maxh_table = self.table_comparisons[self.table_comparisons['guess_str'] == maxh_table_str]['guess'].iloc[0]

        return self.maxh_table

    def next_table_guess(self, current_guess: tuple, table_colors: list):
        """Given a current guess and the resulting table colors, filter the possible rivers to those that match the colors.

        Args:
            current_guess (tuple): A tuple of 5 Card objects representing the current guess.
        ...
        if not getattr(self, "possible_rivers", None):
            raise ValueError("No possible rivers calculated. Please run solve() first.")
        if len(current_guess) != 5 or not all(isinstance(card, Card) for card in current_guess):
            raise ValueError("Current guess must be a tuple of exactly 5 Card objects.")
        if not isinstance(table_colors, list) or len(table_colors) != 5:
            raise ValueError("Table colors must be a list of 5 colors for each card in the table.")

        possible_outcomes = self.comparisons_matrix[str(current_guess)]
        
        current_guess_list = [str(current_guess[i]) + table_colors[i] for i in range(5)]
        flop_set = set(current_guess_list[:3])
        turn_set = set([current_guess_list[3]])
        river_set = set([current_guess_list[4]])
        current_guess_sets = (flop_set, turn_set, river_set)

        possible_outcomes_filtered = possible_outcomes[possible_outcomes == current_guess_sets].index
        comparisons_filtered = self.table_comparisons[self.table_comparisons['guess_str'].isin(possible_outcomes_filtered)]
        self.possible_rivers = list(comparisons_filtered['guess'].unique())
    ...
    def print_game(self, river: tuple):
